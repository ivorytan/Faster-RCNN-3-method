...
def create_model(num_classes):
    ...
    backbone = MobileNetV2().features # 随机初始化进行VOC训练
...
def main():
    ...
    torch.save(model.state_dict(), "./save_weights/random-pretrain.pth")
    ...
    for epoch in range(init_epochs, num_epochs+init_epochs, 1):
        ...
        if epoch in range(num_epochs+init_epochs)[-5:]:
            save_files = {
                ...
                'epoch': epoch}
            torch.save(save_files, "./save_weights/random-model-{}.pth".format(epoch))
